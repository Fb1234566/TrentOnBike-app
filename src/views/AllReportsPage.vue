...
const aggiornaMarkers = async () => {
  try {
    ...
    Map.clearMarkers(); // Cancella i marker precedenti
    ...
    for (const segnalazione of segnalazioni.value) {
      if (
        ...
        await Map.addMarker([lng, lat]); // Aggiunge un marker rosso alla mappa
      } else {
        console.warn(
    ...
    Map.getMap().fitBounds(bounds, { padding: 50 }); // Applica il bound per adattare la mappa
  } catch (error) {
...
const initMappa = async () => {
  try {
    ...
    await Map.create(posizioneStandard, 'mappaLista'); // Inizializza la mappa senza bounds
...
#mappaLista {
  ...
  top: 0;
  width: 100%;
  height: 70vh; /* Altezza fissa per lasciare spazio in basso */
